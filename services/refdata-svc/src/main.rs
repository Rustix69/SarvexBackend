...
async fn main() -> Result<()> {
    ...
    sqlx::query("SELECT 1")
        .execute(&pool)
        .await
        .context("refdata database readiness check failed")?;
...
async fn healthz() -> impl IntoResponse {
    (
        StatusCode::OK,
        Json(serde_json::json!({ "status": "ok", "service": "refdata-svc" })),
    )
}
...
async fn readyz(State(state): State<AppState>) -> impl IntoResponse {
    match sqlx::query("SELECT 1").execute(&state.pool).await {
        Ok(_) => (
            StatusCode::OK,
            Json(serde_json::json!({ "status": "ready", "service": "refdata-svc" })),
        ),
        Err(error) => (
...
impl RefData for RefDataService {
    ...
    ) -> Result<Response<Contract>, Status> {
        let ticker = request.into_inner().ticker;
        let row = sqlx::query(CONTRACT_SELECT)
            .bind(&ticker)
            .fetch_optional(&self.pool)
            .await
            .map_err(internal)?;
        match row {
            Some(row) => Ok(Response::new(contract_from_row(&row))),
            None => Err(Status::not_found("contract not found")),
        }
    }
    ...
    async fn list_contracts(
        ...
        if request.state != 0 {
            let state = contract_state_name(request.state)
                .ok_or_else(|| Status::invalid_argument("invalid contract state"))?;
    ...
    async fn transition_state(
        ...
        let request = request.into_inner();
        let new_state = contract_state_name(request.new_state)
            .ok_or_else(|| Status::invalid_argument("invalid contract state"))?;
        let mut tx = self.pool.begin().await.map_err(internal)?;
        let old =
            sqlx::query("SELECT state::text FROM refdata.contracts WHERE ticker = $1 FOR UPDATE")
                .bind(&request.ticker)
    ...
    async fn upsert_contract(
        &self,
        _request: Request<UpsertContractRequest>,
    ) -> Result<Response<Contract>, Status> {
        Err(Status::unimplemented(
            "contract upsert is not enabled in Phase 01",
        ))
    }
...
fn contract_from_row(row: &sqlx::postgres::PgRow) -> Contract {
    Contract {
        ...
        underlying: row
            .get::<Option<String>, _>("underlying")
            .unwrap_or_default(),
        tick_size: row.get("tick_size"),
        min_price_ticks: row.get("min_price_ticks"),
        max_price_ticks: row.get("max_price_ticks"),
        lower_bound_ticks: row
            .get::<Option<i64>, _>("lower_bound_ticks")
            .unwrap_or_default(),
        upper_bound_ticks: row
            .get::<Option<i64>, _>("upper_bound_ticks")
            .unwrap_or_default(),
        multiplier_micro_usdc: row
            .get::<Option<i64>, _>("multiplier_micro_usdc")
            .unwrap_or_default(),
        ...
        expected_resolution_at: timestamp(row.get("expected_resolution_at")),
        settlement_source: row
            .get::<Option<String>, _>("settlement_source")
            .unwrap_or_default(),
        oracle_policy: row
            .get::<Option<String>, _>("oracle_policy")
            .unwrap_or_default(),
...
fn optional_timestamp(value: Option<DateTime<Utc>>) -> Option<Timestamp> {
    value.map(|v| Timestamp {
        seconds: v.timestamp(),
